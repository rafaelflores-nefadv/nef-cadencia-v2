...
class ToolsReadTests(TestCase):
    ...
    def test_get_productivity_analytics_filters_only_active_agents(self):
        self.agent_3.ativo = False
        self.agent_3.save(update_fields=["ativo", "updated_at"])
        ...
        ranking_names = [item["agent_name"] for item in result["ranking"]]
        self.assertNotIn("Carla", ranking_names)
        self.assertEqual(result["only_active"], True)
        self.assertIn("active_filter_applied", result["diagnostics"])
    ...
    def test_get_agents_listing_returns_registered_agents(self):
        result = get_agents_listing()

        names = [item["agent_name"] for item in result["items"]]
        self.assertIn("Alice", names)
        self.assertIn("Bruno", names)
        self.assertIn("Carla", names)
        self.assertEqual(result["reason_no_data"], None)

    def test_get_agents_listing_filters_only_active(self):
        self.agent_3.ativo = False
        self.agent_3.save(update_fields=["ativo", "updated_at"])

        result = get_agents_listing(only_active=True)
    ...
    def test_get_productivity_analytics_result_is_json_serializable(self):
        result = get_productivity_analytics(
            year=2026,
            month=1,
            metric="productivity",
        ...
        self.assertIn("\"ranking\"", serialized)


class ProductivityAnalyticsAvailabilityInvestigationTests(TestCase):
    def test_productivity_analytics_requires_telemetry_not_only_agent_registry(self):
        today = timezone.localdate()
        Agent.objects.create(cd_operador=9901, nm_agente="Sem Telemetria")
        ...
        self.assertEqual(result["ranking"], [])
        self.assertEqual(result["summary"]["total_agents_considered"], 0)
        self.assertEqual(result["reason_no_data"], "filter_without_match")
        self.assertIn("filter_without_match", result["diagnostics"])
