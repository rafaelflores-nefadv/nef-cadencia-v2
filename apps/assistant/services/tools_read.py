...
def _coerce_optional_bool(value) -> bool | None:
    ...
    if normalized in {"true", "1", "yes", "sim", "ativo", "ativos"}:
        return True
    if normalized in {"false", "0", "no", "nao", "all", "todos"}:
        return False
    return None
...
def get_agents_listing(
    *,
    ...
    try:
        parsed_only_active = _coerce_optional_bool(only_active)
        normalized_search = str(search or "").strip()
        try:
            parsed_limit = int(limit or 50)
        except (TypeError, ValueError):
            parsed_limit = 50
        normalized_limit = max(1, min(parsed_limit, 200))

        agents_qs = Agent.objects.all().order_by("cd_operador")
        if parsed_only_active is True:
            agents_qs = agents_qs.filter(ativo=True)
        if normalized_search:
            search_filter = (
                Q(nm_agente__icontains=normalized_search)
                | Q(nm_agente_code__icontains=normalized_search)
                | Q(email__icontains=normalized_search)
            )
            if normalized_search.isdigit():
                search_filter = search_filter | Q(cd_operador=int(normalized_search))
            agents_qs = agents_qs.filter(search_filter)

        total_found = agents_qs.count()
        rows = list(
            agents_qs.values(
                "id",
                "cd_operador",
        ...
        items = [
            {
                ...
                "agent_name": row.get("nm_agente") or row.get("nm_agente_code") or f"Operador {row['cd_operador']}",
                "agent_code": row.get("nm_agente_code") or "",
        ...
        if not items:
            if parsed_only_active is True:
                reason_no_data = "no_active_agents_found"
            elif normalized_search:
                reason_no_data = "no_agents_match_filter"
            else:
                reason_no_data = "no_agents_registered"

        diagnostics = ["agent_registry_used"]
        if parsed_only_active is True:
            diagnostics.append("active_filter_applied")
        if normalized_search:
            diagnostics.append("search_filter_applied")

        logger.info(
            "assistant.get_agents_listing only_active=%s search=%s limit=%s total_found=%s returned=%s reason_no_data=%s",
            parsed_only_active,
            normalized_search,
            normalized_limit,
            total_found,
        ...
        return {
            "only_active": parsed_only_active,
        ...
        logger.exception(
            "assistant.get_agents_listing_failed only_active=%s search=%s limit=%s",
            only_active,
            search,
            limit,
        )
    ...
    *,
    ...
    try:
        ...
        if parsed_only_active is True:
            active_operator_ids = {
                int(cd_operador)
                for cd_operador in Agent.objects.filter(ativo=True).values_list("cd_operador", flat=True)
                if cd_operador is not None
            }
            diagnostics.append("active_filter_applied")
            if active_operator_ids:
                workday_rows = [
                    row
                    for row in workday_rows
                    if row.get("cd_operador") is not None and int(row["cd_operador"]) in active_operator_ids
                ]
                stats_qs = stats_qs.filter(cd_operador__in=active_operator_ids)
                events_qs = events_qs.filter(cd_operador__in=active_operator_ids)
            else:
                workday_rows = []
                stats_qs = stats_qs.none()
        ...
        logger.warning(
            (
                ...
                "start_date=%s end_date=%s metric=%s group_by=%s ranking_order=%s only_active=%s "
        ...
        logger.exception(
            "assistant.get_productivity_analytics_failed date_from=%s date_to=%s year=%s month=%s period_key=%s metric=%s group_by=%s ranking_order=%s only_active=%s limit=%s",
            date_from,
            date_to,
            year,
            month,
