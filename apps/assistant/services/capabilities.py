...
CAPABILITY_DEFINITIONS = (
    ...
    CapabilityDefinition(
        capability_id="agent_listing_query",
        category=CONSULTA_SUPORTADA,
        description="listar agentes cadastrados ou ativos no sistema",
        patterns=(
            r"\b(liste|listar|mostra|mostre|mostrar|quais|quais sao|quem sao)\b.*\b"
            r"(agente|agentes|operador|operadores)\b.*\b"
            r"(cadastrad[a-z]*|existem|existentes|ativos?|ativo|sistema)\b",
            r"\b(agente|agentes|operador|operadores)\b.*\b"
            r"(cadastrad[a-z]*|existentes?|ativos?)\b",
            r"^(liste|listar|mostra|mostre|mostrar)\s+(os\s+)?(agentes|operadores)$",
            r"^(quais|quais sao)\s+(os\s+)?(agentes|operadores)$",
        ),
        query_tools=(TOOL_GET_AGENTS_LISTING,),
        unsupported_response=ASSISTANT_UNSUPPORTED_CAPABILITY_RESPONSE,
        no_data_response=ASSISTANT_NO_AGENTS_RESPONSE,
        query_failure_response=ASSISTANT_QUERY_FAILURE_RESPONSE,
        action_failure_response=ASSISTANT_ACTION_FAILURE_RESPONSE,
        unverified_response=ASSISTANT_UNVERIFIED_RESPONSE,
    ),
    CapabilityDefinition(
        ...
        patterns=(
            r"\b(qual|quais|quem|ranking|top|mostra|mostre|mostrar|liste|listar)\b.*\b"
            ...
            r"\b(quem|quais|qual)\b.*\b(produziu|produziram|produz[a-z]*)\b.*\b"
            r"(agente|agentes|operador|operadores|hoje|ontem|semana|mes|periodo)\b",
        ),
...
def tool_result_has_data(tool_name: str, result: dict) -> bool | None:
    ...
    if tool_name == TOOL_GET_AGENTS_LISTING:
