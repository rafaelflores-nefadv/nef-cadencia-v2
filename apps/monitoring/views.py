...
from .services.dashboard_analytics import (
    ...
    build_pause_type_rankings,
    build_productivity_evolution,
    build_risk_radar_dimensions,
    resolve_productivity_seconds,
)
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def _build_gamified_leaderboard(leaderboard_agents: list[dict], operation_average: float) -> list[dict]:
        ...
        for position, item in enumerate(leaderboard_agents, start=1):
            ...
            if agent_score >= 85:
                badges.append("Consistência alta")
            if agent_score > average:
                badges.append("Acima da média")
            cards.append(
                {
                    **item,
                    "position": position,
    ...
    def get_context_data(self, **kwargs):
        ...
        pause_type_by_time, pause_type_by_count = build_pause_type_rankings(
            pause_events_qs=pause_events_qs,
            period_end_dt=period_end,
        )
        executive_timeline = build_operational_timeline(
        ...
        event_span_by_operator = self._estimate_event_span_seconds_per_operator(
            events_day_qs=events_day_qs,
            period_end=period_end,
        )
        ...
        if total_logged_time <= 0:
            total_logged_time = sum(event_span_by_operator.values())
        ...
        stats_warning = stats_agents_count == 0 and (total_events_day > 0 or workday_count > 0)
        ...
        no_activity_agents = [
            {
                ...
                "nm_agente": item.get("nm_agente"),
            }
            for item in active_agents_without_activity
        ]
        productivity_evolution = build_productivity_evolution(
            period_filter=period_filter,
            events_qs=events_day_qs,
            pause_events_qs=pause_events_qs,
        ...
        risk_radar_dimensions = build_risk_radar_dimensions(
            operator_metrics=operator_metrics,
            no_activity_agents=no_activity_agents,
        ...
        if pause_data_state == "missing":
            pause_block_message = (
                f"Sem dados de pausas importados para o período {period_filter.period_label}. "
                f"Execute: {command_snippets['import_pause']}"
            )
        elif pause_data_state == "zero":
            pause_block_message = "Total de pausas: 0 (dados presentes no período)."
        ...
        context.update(
            {
                ...
                "operator_metrics": operator_metrics,
                "productivity_chart_agents": top_productivity[:10],
                "risk_agents": risk_agents,
                "no_activity_agents": no_activity_agents,
                "risk_radar_dimensions": risk_radar_dimensions,
                "executive_pause_radar": risk_radar_dimensions,
                "alerts": alerts,
                "alert_totals": alert_totals,
                ...
                "productivity_evolution": productivity_evolution,
...
class DashboardPipelineView(DashboardView):
    ...
    @staticmethod
    def _estimate_event_span_seconds_per_operator(events_day_qs, period_end):
        now = timezone.now()
        close_time = min(period_end, now)
        bounds_by_operator: dict[int, dict[str, datetime | None]] = {}

        for item in events_day_qs.values("cd_operador", "dt_inicio", "dt_fim"):
    ...
    def _build_operator_metrics(
        ...
        for cd_operador in sorted(operator_ids):
            ...
            productive_seg, improductive_basis_seg, productivity_basis = resolve_productivity_seconds(
    ...
    def _build_relevant_events_by_operator(events_day_qs):
        rows = list(events_day_qs.values("cd_operador").annotate(total=Count("id")))
        return {
...
class DashboardRebuildStatsView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(f"{reverse('dashboard-productivity')}?{urlencode({'data_ref': selected_date.isoformat()})}")
...
class DashboardAgentsIAView(LoginRequiredMixin, TemplateView):
    template_name = "monitoring/dashboard_agents_ia.html"
...
class DashboardDataReceivingView(LoginRequiredMixin, TemplateView):
    template_name = "monitoring/dashboard_data_receiving.html"
...
class DashboardDataSendingView(LoginRequiredMixin, TemplateView):
    template_name = "monitoring/dashboard_data_sending.html"
