...
class DashboardViewTests(TestCase):
    ...
    def test_rankings_show_10_when_more_than_10_operators(self):
        ...
        self.assertContains(response, "Top 10 por tempo improdutivo (10)")
        self.assertContains(response, "Top 10 agentes mais produtivos (10)")
        self.assertEqual(html.count('data-ranking-row="pause-time"'), 10)
        self.assertEqual(html.count('data-ranking-row="pause-count"'), 10)
        self.assertEqual(html.count('data-ranking-row="productivity"'), 10)
    ...
    def test_risk_block_shows_10_when_more_than_10_operators(self):
        ...
        self.assertContains(response, "Agentes com maior risco (10)")
        self.assertEqual(html.count('data-risk-row="agent"'), 10)
    ...
    def test_risk_block_shows_3_when_three_operators(self):
        ...
        self.assertContains(response, "Agentes com maior risco (3)")
        self.assertEqual(html.count('data-risk-row="agent"'), 3)
    ...
    def test_risk_block_shows_1_when_one_operator(self):
        ...
        self.assertContains(response, "Agentes com maior risco (1)")
        self.assertEqual(html.count('data-risk-row="agent"'), 1)
    ...
    def test_logged_agent_without_relevant_activity_has_high_risk(self):
        self.client.force_login(self.user)
        ...
        self.assertGreaterEqual(risk_by_cd[410]["risk_score"], 250)
        self.assertEqual(risk_by_cd[410]["primary_reason"], "sem atividade")
        self.assertEqual(risk_by_cd[410]["tempo_improdutivo_hhmm"], "00:00:00")
        self.assertEqual(risk_by_cd[410]["taxa_ocupacao_pct"], 0.0)
        self.assertEqual(risk_by_cd[410]["operator_id"], 410)
        self.assertEqual(risk_by_cd[410]["score"], risk_by_cd[410]["risk_score"])
    ...
    def test_rankings_show_3_when_three_operators(self):
        ...
        self.assertContains(response, "Top 10 por tempo improdutivo (3)")
        self.assertContains(response, "Top 10 agentes mais produtivos (3)")
        self.assertEqual(html.count('data-ranking-row="pause-time"'), 3)
        self.assertEqual(html.count('data-ranking-row="pause-count"'), 3)
        self.assertEqual(html.count('data-ranking-row="productivity"'), 3)
    ...
    def test_rankings_show_1_when_one_operator(self):
        ...
        self.assertContains(response, "Top 10 por tempo improdutivo (1)")
        self.assertContains(response, "Top 10 agentes mais produtivos (1)")
        self.assertEqual(html.count('data-ranking-row="pause-time"'), 1)
        self.assertEqual(html.count('data-ranking-row="pause-count"'), 1)
        self.assertEqual(html.count('data-ranking-row="productivity"'), 1)
    ...
    def test_dashboard_with_stats_populates_summary_and_rankings(self):
        ...
        response = self.client.get(reverse("dashboard"), {"data_ref": self.day.isoformat()})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Resumo executivo do dia")
    ...
    def test_dashboard_rejects_invalid_custom_date_range(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["total_events_day"], 3)
        self.assertEqual(response.context["filter_warning"], "Data inicial nao pode ser maior que data final.")
    ...
    def test_dashboard_requires_year_when_filtering_by_month_only(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["total_events_day"], 3)
        self.assertEqual(response.context["filter_warning"], "Filtro por mes exige ano.")
        self.assertEqual(response.context["applied_period_label"], "05/03/2026")
    ...
    def test_ranking_title_counts_match_rendered_rows(self):
        ...
        self.assertContains(response, f"Top 10 por tempo improdutivo ({top_pause_time_len})")
        self.assertContains(response, f"Top 10 por qtd de pausas improdutivas ({top_pause_count_len})")
        self.assertContains(response, f"Top 10 agentes mais produtivos ({top_productivity_len})")
        self.assertEqual(html.count('data-ranking-row="pause-time"'), top_pause_time_len)
        self.assertEqual(html.count('data-ranking-row="pause-count"'), top_pause_count_len)
        self.assertEqual(html.count('data-ranking-row="productivity"'), top_productivity_len)
    ...
    def test_dashboard_without_pause_dataset_shows_hint_and_na(self):
        ...
        self.assertContains(response, "Sem dados de pausas importados")
        self.assertContains(response, "python manage.py import_lh_pause_events --date 2026-03-05")
    ...
    def test_dashboard_shows_distribution_and_alert_for_unclassified_pauses(self):
        ...
        self.assertIn("Tempo Nao Classificado", labels)
