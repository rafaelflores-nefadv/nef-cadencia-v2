...
class SettingsChildPagesTests(TestCase):
    ...
    def test_settings_assistant_context_exposes_guardrail_values(self):
        self.client.login(username='admin', password='testpass123')
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['assistant_guardrails']['scope_guardrail_enabled'])
        self.assertFalse(response.context['assistant_guardrails']['capability_guardrail_enabled'])
        self.assertFalse(response.context['assistant_guardrails']['output_scope_guardrail_enabled'])
        self.assertTrue(response.context['assistant_guardrails']['output_truthfulness_guardrail_enabled'])

    def test_settings_assistant_post_persists_guardrail_values(self):
        self.client.login(username='admin', password='testpass123')

        response = self.client.post(
            reverse('settings-assistant'),
            data={
                "scope_guardrail_enabled": "on",
                # capability_guardrail_enabled sem valor -> False
                "output_scope_guardrail_enabled": "on",
                # output_truthfulness_guardrail_enabled sem valor -> False
            },
        )

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('settings-assistant'))

        scope_config = SystemConfig.objects.get(config_key=ASSISTANT_SCOPE_GUARDRAIL_ENABLED_KEY)
        capability_config = SystemConfig.objects.get(
            config_key=ASSISTANT_CAPABILITY_GUARDRAIL_ENABLED_KEY
        )
        output_scope_config = SystemConfig.objects.get(
            config_key=ASSISTANT_OUTPUT_SCOPE_GUARDRAIL_ENABLED_KEY
        )
        output_truthfulness_config = SystemConfig.objects.get(
            config_key=ASSISTANT_OUTPUT_TRUTHFULNESS_GUARDRAIL_ENABLED_KEY
        )
        self.assertEqual(scope_config.config_value, "true")
        self.assertEqual(capability_config.config_value, "false")
        self.assertEqual(output_scope_config.config_value, "true")
        self.assertEqual(output_truthfulness_config.config_value, "false")
        self.assertEqual(scope_config.value_type, SystemConfigValueType.BOOL)
        self.assertEqual(capability_config.value_type, SystemConfigValueType.BOOL)
        self.assertEqual(output_scope_config.value_type, SystemConfigValueType.BOOL)
        self.assertEqual(output_truthfulness_config.value_type, SystemConfigValueType.BOOL)
