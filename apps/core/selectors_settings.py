...
ASSISTANT_INTERNAL_TEMPLATE_NAMES = {
    "pause_overflow",
    "pause_warning",
    "supervisor_alert",
}
...
def _managed_message_templates_queryset():
    from apps.messaging.choices import ChannelChoices
    from apps.messaging.models import MessageTemplate

    return MessageTemplate.objects.exclude(
        channel=ChannelChoices.CHATSEGURO,
        name__in=ASSISTANT_INTERNAL_TEMPLATE_NAMES,
    )
...
def get_message_templates_stats() -> dict:
    ...
    """
    templates = _managed_message_templates_queryset()
...
def get_recent_template_changes(limit: int = 5) -> QuerySet:
    ...
    """
    return _managed_message_templates_queryset().order_by('-updated_at')[:limit]
